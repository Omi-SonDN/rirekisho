    ...
    <ul class="">
        @can('Applicant')
            <li class="nav-link"></li>
                <li class="nav-link"><a href="#" data-toggle="collapse" data-target="#submenu1">Danh sách hồ sơ <i class="fa fa-angle-down"></i></a>
                    <ul class="nav collapse" id="submenu1">
                        ...
                        @if (count($CV))
                            @if(count($CV) > 1)
                                @foreach($CV as $items)
                                    @if ($items->type_cv)
                                        <li class=""><a href="{{\URL('CV/create/upload/'.$items->Hash)}}">Xem CV đính kèm</a></li>
                                    @else
            ...
            @foreach($list as $item)
                <li class="track-list">
                    <label style="display: table;width:100%;">
                        ...
                        <a href="{{url('CV',[$item])}}"
                           style="display: table-cell; width: 145px; font-size: 15px;font-style: normal; font-weight: 400;">{{$item->User->Last_name}} {{$item->User->First_name}}</a>
        ...
        @can('SuperAdmin')
        <li class="nav-link"><a href="#" data-toggle="collapse" data-target="#submenu-sa">Cài đặt <i class="fa fa-angle-down"></i></a>
